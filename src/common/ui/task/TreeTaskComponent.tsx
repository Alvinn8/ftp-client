...
const TreeTaskComponent: React.FC<TaskProps> = ({
    treeTask,
    onShowDetails,
}) => {
    ...
    return (
        ...
        >
            <div className="d-flex gap-1">
                <strong className="me-auto text-color text-normal text-truncate">
                    {treeTask.title}
                </strong>
                ...
                <>
                    ...
                    <div className="d-flex justify-content-between gap-3 text-xs text-muted-color text-smaller">
                        {progress.totalFiles > 1 && (
                            <span>
                                {progress.completedFiles} of{" "}
                                {progress.totalFiles} files
                            </span>
                        )}
                        {progress.totalFileSize && (
                            <span>
                                {formatByteSize(progress.completedFileSize)} /{" "}
                                {formatByteSize(progress.totalFileSize)}
                            </span>
                        )}
                    </div>
                </>
            ...
            <div className="d-flex flex-wrap gap-1 mt-2">
                ...
                <Button
                    ...
                    size="small"
                    severity={
                        status === TaskStatus.ERROR ? "danger" : "secondary"
                    }
