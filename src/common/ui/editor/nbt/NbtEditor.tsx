...
const NbtEditor = () => {
    ...
    const handleSave = () => {
        ...
        sanityCheckNbt(blob, nbt)
            ...
            .catch((err) => {
                Dialog.message(
                    "Error",
                    "Failed to save NBT file. Please contact support if you wish to modify this file. " +
                        formatError(err),
                );
            });
    };
    ...
    const bedrockLevelDat =
        nbt.editionData.edition === "bedrock" &&
    ...
    return (
        <div className="grid">
            <div className="ms-5 nbt-editor">
                <div className="mb-2">
                    <span className="badge bg-secondary mt-3">
                        {editionText(nbt)}
                    </span>
                    {!allowSaving && (
                        <div
                            className="text-bg-warning rounded p-2 my-3"
                            style={{ maxWidth: "400px", fontSize: "0.75em" }}
                        >
                            This NBT file cannot be modified and saved. Please
                            contact support if you wish to modify this file.
                        </div>
                    )}
                </div>
                <UiNbtTag
...
function editionText(nbtData: NbtData) {
    let txt =
        nbtData.editionData.edition === "java"
            ? "Java Edition"
            : "Bedrock Edition";
    if (
        nbtData.editionData.edition === "bedrock" &&
        (nbtData.editionData as BedrockEdition).isLevelDat
    ) {
        txt +=
            " level.dat version " +
            (nbtData.editionData as BedrockLevelDat).headerVersion;
    } else {
        txt += " NBT";
    }
