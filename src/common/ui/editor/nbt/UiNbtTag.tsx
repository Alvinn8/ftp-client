...
import {
    contextMenuForString,
    contextMenuForNumber,
    contextMenuForArray,
    ParentData,
} from "./nbtContextMenu";
interface Props {
    ...
    render() {
        ...
        if (tag instanceof NumberNbtTag) {
            return (
                <NbtTagContainer
                    ...
                    populator={contextMenuForNumber(
                        tag,
                        this.props.parent,
                        reRenderUi,
                    )}
                >
                    <UiNbtNumberComponent
                        value={tag.toString()}
                        type={tag.getTypeChar()}
                    />
                </NbtTagContainer>
            );
        }
        if (tag instanceof NbtString) {
            return (
                <NbtTagContainer
                    label={this.props.children}
                    populator={contextMenuForString(
                        tag,
                        this.props.parent,
                        reRenderUi,
                    )}
        ...
        if (tag instanceof ArrayNbtTag) {
            return (
                <NbtTagContainer
                    label={null}
                    populator={contextMenuForArray(
                        tag,
                        this.props.parent,
                        reRenderUi,
                    )}
                >
                    <UiNbtArray tag={tag}>{this.props.children}</UiNbtArray>
                </NbtTagContainer>
            );
        }
        return (
