        ...
        <div>
            <div className="logs">
                {lines.map((line, index) => {
                    return (
                        <span className={className(line)} key={index}>
                            {line.text}
                        </span>
                    );
                })}
...
function parseLogs(text: string): Line[] {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        const errorIndices = errorKeywords
            ...
            .filter((idx) => idx !== -1);
        const firstErrorIdx =
            errorIndices.length > 0 ? Math.min(...errorIndices) : -1;
        const warnIdx = currentLower.indexOf("warn");

        if (
            /(^|\s)at ([ a-zA-Z0-9\.<>_\-$/@^#\\+]+)\(.+\)/.exec(currentLower)
        ) {
            line.level = "error-stacktrace";
        } else if (
            warnIdx !== -1 &&
            (firstErrorIdx === -1 || warnIdx < firstErrorIdx)
        ) {
            line.level = "warning";
...
function formatLine(text: string): React.JSX.Element {
    if (
        !text.includes(SECTION_COLOR) &&
        !ANSI_COLOR_REGEX.exec(text) &&
        !text.includes(UNKNOWN_COLOR)
    ) {
        return <>{text}</>;
    }
    ...
    function savePart(skip: number) {
        ...
        const className = [...formatting.values()]
            .map((f) => "line-format-" + f)
            .join(" ");
        parts.push(
            <span className={className} style={{ color }} key={parts.length}>
                {partText}
            </span>,
        );
    }
