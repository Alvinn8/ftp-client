...
export interface TreeTaskHandler<T> {
    ...
    ): MaybePromise<HandlerAction | void>;
    /**
...
export class TreeTask<T = unknown> extends EventEmitter {
    ...
    private checkCancellationCompletion() {
        ...
        if (this.activeTasks.length === 0) {
            this.callFinalHandler("cancelled")
                .then(() => {
    ...
    private async callFinalHandler(handlerType: "done" | "cancelled") {
        ...
        while (attempt <= this.maxAttempts) {
            let success = false;
            await this.session.addToPoolQueue(
                Priority.LARGE_TASK,
                async (connection) => {
                    try {
                        ...
                        console.error(
                            `Error in final handler '${handlerType}' (attempt ${attempt}):`,
                            error,
                        );
                        attempt++;
        ...
        console.error(
            `Failed to call final handler '${handlerType}' after ${this.maxAttempts} attempts.`,
        );
    }
    ...
    updateProgress() {
        this.progress.value =
            this.count.completedFiles + this.count.completedDirectories;
    ...
    findErrorTasks(
        fileTree: FileTree<T>,
        acc: (FileTree<T> | FileTreeFile<T>)[],
    ) {
        if (
            fileTree.getBeforeStatus() === Status.ERROR ||
            fileTree.getAfterStatus() === Status.ERROR
        ) {
            acc.push(fileTree);
        }
        for (const entry of fileTree.getEntries()) {
            if (
                entry instanceof FileTreeFile &&
                entry.getStatus() === Status.ERROR
            ) {
                acc.push(entry);
    ...
    addNextSubTask(session: FTPSession) {
        ...
        if (
            ...
            session.getConnectionPool().getTargetConnectionCount()
        ) {
        ...
        if (subTask.type === "file") {
            ...
            this.emit("activeTasksChange", this.activeTasks);
            session
                .addToPoolQueue(Priority.LARGE_TASK, async (connection) => {
                    try {
                        const action = await this.handler.file(
                            subTask.node,
                            connection,
                        );
                        ...
                        if (subTask.node.getStatus() === Status.DONE) {
                            ...
                            this.count.completedFileSize +=
                                subTask.node.fileSize() || 0;
                        }
                        this.updateProgress();
                    } catch (error) {
                        ...
                        connection.close();
                        console.error(
                            "Error processing file " + subTask.node.name + ": ",
                            error,
                        );
            ...
            session
                .addToPoolQueue(Priority.LARGE_TASK, async (connection) => {
                    try {
                        const action = await this.handler.beforeDirectory(
                            subTask.node,
                            connection,
                        );
                        if (action && action.type === "pause_to_resume") {
                        ...
                        connection.close();
                        console.error(
                            "Error processing before directory " +
                                subTask.node.path +
                                ": ",
                            error,
                        );
                ...
                .catch(
                    unexpectedErrorHandler(
                        "TreeTask.addNextSubTask.beforeDirectory",
                    ),
                );
            ...
            session
                .addToPoolQueue(Priority.LARGE_TASK, async (connection) => {
                    try {
                        const action = await this.handler.afterDirectory(
                            subTask.node,
                            connection,
                        );
                        if (action && action.type === "pause_to_resume") {
                            ...
                            for (const entry of subTask.node.getEntries()) {
                                if (
                                    ...
                                    entry.getStatus() !== Status.CANCELLED
                                ) {
                                    allDone = false;
                                } else if (
                                    entry instanceof FileTree &&
                                    ((entry.getBeforeStatus() !== Status.DONE &&
                            ...
                            if (allDone) {
                                ...
                                subTask.node.setError(
                                    new Error(
                                        "Not all entries in this folder were processed successfully. Please try again.",
                                    ),
                                );
                                subTask.node.retry();
                            }
                        ...
                        connection.close();
                        console.error(
                            "Error processing after directory " +
                                subTask.node.path +
                                ": ",
                            error,
                        );
                ...
                .catch(
                    unexpectedErrorHandler(
                        "TreeTask.addNextSubTask.afterDirectory",
                    ),
                );
        }
    }
    ...
    private walkFileTree(fileTree: FileTree<T>): SubTask<T> | null {
        ...
        const shouldProcessDirectory =
            !isRootDirectory || this.options.processRootDirectory;

        if (
            shouldProcessDirectory &&
            fileTree.getBeforeStatus() === Status.PENDING
        ) {
            return {
        ...
        if (
            shouldProcessDirectory &&
            fileTree.getBeforeStatus() !== Status.DONE
        ) {
            return null;
        }
        ...
        for (let i = 0; i < entries.length; i++) {
            ...
            if (entry instanceof FileTree) {
                ...
                if (
                    (entry.getBeforeStatus() !== Status.DONE &&
                        entry.getBeforeStatus() !== Status.CANCELLED) ||
                    (entry.getAfterStatus() !== Status.DONE &&
                        entry.getAfterStatus() !== Status.CANCELLED)
                ) {
                    allDone = false;
                }
        ...
        if (
            allDone &&
            shouldProcessDirectory &&
            fileTree.getAfterStatus() === Status.PENDING
        ) {
            return {
                type: "afterDirectory",
                node: fileTree,
        ...
        if (
            allDone &&
            isRootDirectory &&
            !this.options.processRootDirectory &&
            fileTree.getAfterStatus() === Status.PENDING
        ) {
