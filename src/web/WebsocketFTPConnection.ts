...
export default class WebsocketFTPConnection implements FTPConnection {
    ...
    async connectToWebsocket() {
        ...
        await new Promise<void>((resolve, reject) => {
            ...
            this.websocket.addEventListener("open", () => {
                this.websocket.send(
                    `handshake ${PROTOCOL_TYPE} ${PROTOCOL_VERSION}`,
                );
                resolve();
            });
            ...
            this.websocket.addEventListener("close", (e) => {
                ...
                if (e.code != 1000 && e.code != 1005 && e.code != 1006)
                    message = "Connection closed: " + e.code + " " + e.reason;

                this.rejectPendingReplies(
                    new ConnectionClosedError(message, e.code, e.reason),
                );

                const sessionStore = useSession.getState();
                const taskManager = sessionStore.hasSession()
                    ? sessionStore.getSession().taskManager
                    : null;
                const hasTask =
                    taskManager && taskManager.getTreeTasks().length > 0;
                if (
                    message == "Connection closed" &&
                    (!hasTask || this.closing)
                )
                    return;
                addMessage({
                    color: "danger",
    ...
    async list(path: string): Promise<FolderEntry[]> {
        ...
        return (await this.send(Packets.List, { path })).files.map(
            (fileInfo) => {
        ...
        const intervalId = setInterval(() => {
            ...
            this.isConnected()
                .then((val) => {
                    if (!val) {
                        console.warn(
                            "FTP not connected while still downloading.",
                        );
                    }
                })
                .catch((e) => {
        ...
        if (response.downloadId) {
            ...
            return await new Promise((resolve, reject) => {
                ...
                xhr.addEventListener(
                    "progress",
                    progressTracker("download", folderEntry.path, progress),
                );
                xhr.addEventListener("readystatechange", (event) => {
                    if (xhr.readyState === XMLHttpRequest.DONE) {
                        if (
                            ...
                            reject(
                                new Error(
                                    `Failed to download large file. HTTP ${xhr.status} ${xhr.statusText}`,
                                ),
                            );
                        }
                    }
                ...
                xhr.addEventListener("error", (event) => {
                    largeFileOperationStore.setValue(null);
                    reject(
                        new Error("Network error while downloading large file"),
                    );
                });
                xhr.open("GET", url);
                xhr.send();
